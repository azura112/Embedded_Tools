...
#
#        gcc -std=c99 -Wall -Wextra -pedantic -I. -Icore -Ialgorithm -Isys -Iprotocol -Idrivers -Idebug -Iport -Iport/host \
#            -o build/demo.exe core/*.c algorithm/*.c sys/*.c protocol/*.c drivers/*.c debug/*.c \
...
# ─────────────────────────────────────────────────────────────────────────
CC      := gcc
CFLAGS  := -std=c99 -Wall -Wextra -pedantic -I. -Icore -Ialgorithm -Isys -Iprotocol -Idrivers -Idebug -Iport -Iport/host
OBJDIR  := build

CORE_SRC    := core/et_ringbuf.c core/et_queue.c core/et_mempool.c core/et_list.c
ALGO_SRC    := algorithm/et_filter.c
...
LIB_SRC     := $(CORE_SRC) $(ALGO_SRC) $(SYS_SRC) $(PROTO_SRC) $(DRIVERS_SRC) $(DEBUG_SRC)

TEST_SRC := test/et_test.c test/test_ringbuf.c test/test_queue.c test/test_mempool.c \
            test/test_list.c test/test_filter.c \
