...
| v2.5 | 23468 | 24 | 740 | 默认裁剪构建（终值）；+et_modbus_master 入 glob；**et_log 规格解析加固**（域宽/精度/占位解析器 + ET_LOG_FIELD_MAX）+ demo 改域宽格式串（删 24 行逐位拼字符绕行） |
| v2.5 | 33020 | 24 | 2232 | **ET_MODULE_SELFTEST=1 构建**（CI/仿真常态，终值）；同默认行增量来源 |
