 ...
 * @brief   Cortex-M3 启动代码: 向量表 + 数据段初始化 (不依赖 libc crt0)
 *
...
void SysTick_Handler(void);         /* 实现在 port_stm32f103.c */
...
/* 内核异常 16 项 + 外设中断 68 项 (F103 大类); USART1 为 IRQ57 */
...
const isr_fn_t g_vector_table[] = {
    ...
    [16 + 57] = Default_Handler,    /* 73: USART1_IRQHandler(轮询收发, 未用) */
};
