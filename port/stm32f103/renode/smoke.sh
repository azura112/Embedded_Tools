...
# v2.0 单一来源: smoke 消费 docbuild 的 selftest 变体产物
ELF=$(norm "$REPO/build/stm32f103_demo_selftest.elf")
...
# 擦除态填充 (64KB 全 0xFF): 仓库不入库(.gitignore *.bin), 缺失时现场生成
if [ ! -f "$FILL" ]; then
    head -c 65536 /dev/zero | tr '\000' '\377' > "$FILL" || fail "无法生成擦除态填充: $FILL"
fi
