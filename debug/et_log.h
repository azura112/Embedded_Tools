 ...
 *     (此为例外, 见下"使用约定")。
 *   - **`L` + 整数转换(`%Lu`/`%Ld`/`%Lx`)= 未定义行为**(v2.7 `CO-10` 文档限定): C 标准中
 *     长度修饰 `L` **只对浮点转换有定义**; 该组合库一律按"已知不支持 → 消费 `long double`
 *     一次 + 占位"处置。本机(Windows x64, 整型/浮点同槽宽)实测**不错位**(v2.6 评审探针 E13),
 *     但 **x86-64 SysV(Linux)** 下 `long double` 属 memory-class、`va_arg` 可能**不推进**
 *     整数实参槽 → 后继实参仍可能错位 —— 本库**不保证该组合的可移植语义**, 请勿使用;
 *     需要 64 位整数请用 `%lld` / `%llu` / `%llx`。
 *
 * 使用约定 (v2.5 缺陷清偿的直接动因):
