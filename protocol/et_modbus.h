 ...
 *    0x10 = 9 + bytecount), 收齐即处理 —— 低延迟。**v2.7 (CO-5) 起**:
 *    0x10 的字节数域**须先通过合理性校验**(`b[6] == 2*((b[4]<<8)|b[5])`)
 *    才被采信为帧长 —— 不符即视为噪声头部, 不得据此长度前进;
 ...
 *    (CRC 无效 → 按下方"统计口径"计入 `crc_err` 或 `discarded` 后丢弃)。
 *    **不内部取时基**(库惯例)。
 *  - **逐字节重同步(v2.7, HC-2)**: 当缓冲首部**已不可能**是本站请求的起点
 *    (帧首既非本站地址也非广播地址, 或 0x10 的长度域不自洽)时, 当场丢弃 1 字节
 *    并重新判定 —— **不得停等**: 否则一个噪声字节会永久占据缓冲头部, 使其后
 *    紧跟的真请求再也无法成帧(评审 E10 的受害形态)。字节不足 2 个、或功能码
 *    未知时不重同步 —— 前者可能是真帧前缀, 后者须由静默路径界定
 *    (库承诺的"未知功能码回异常 0x01"依赖该路径)。
...
/* 只读统计 —— **加一条件**(v2.7 HC-3; 与实现差集为空, 逐条由 test/test_modbus.c 固定):
 *   frames        CRC 通过 且 (地址匹配 或 广播) 的请求帧(含未知功能码帧, 经静默路径)
 *   responses     已生成应答帧数(异常应答计入)
 *   exceptions    其中异常应答数
 *   crc_err       **仅**当"与本站期望同形"的候选帧 CRC 失败: 帧首 == 本站从站地址
 *                 且 功能码属已支持集合 且 长度域与功能码自洽(0x03/0x04/0x06 = 8 字节;
 *                 0x10 = 9 + b[6] 且 b[6] == 2*qty)。语义 = "本来发往本站、本应被应答
 *                 却坏在路上"的帧数(链路质量指标); **广播帧不计**(无需应答),
 *                 "从未成帧"的噪声字节也不计(v2.7 口径收紧, 见文件头)。
 *   addr_mismatch CRC 通过但帧首既非本站地址也非广播地址(别的从站的帧, 静默丢弃)
 *   discarded     ① "从未成帧"的字节序列(逐字节重同步, 每丢 1 字节计 1);
 *                 ② 缓冲溢出丢整批; ③ 过短残帧(len < 4); ④ 帧长超出 rxcap
 */
typedef struct {
    uint32_t frames;
    uint32_t responses;
