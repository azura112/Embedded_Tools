...
**基线滚动规则(v2.2)**:默认基线 = 最近已发布 MINOR 的快照(当前 `docs/API_INVENTORY_v2.5.md`),
发版时 `--snapshot` 归档新基线后前滚默认值;旧基线(`API_INVENTORY_v2.0.md`~`v2.4.md`)只读保留作历史审计:
...
v2.4 实绩:新增 22 项 / 修改删除 0 项(et_modbus 从站);v2.5 实绩见交付文档(et_log 域宽上限宏 + et_modbus_master)。

### 5.1 结构体字段追加登记区 (附则要求, 逐版累积)

| 版本 | 结构体 | 追加字段 | 理由 | 破坏半径评估 |
|---|---|---|---|---|
| v2.2 | `et_task_t`(sys/et_sched.h) | `last_ms`/`max_ms`(尾部追加) | 任务耗时统计(P4): 每任务 last/max 执行时长, 板上诊断抖动/超时任务的第一手数据 | **调用方重编译即兼容**: 字段在结构体**尾部**追加, 既有字段偏移不变; 实例均为调用方静态分配, 无 ABI 边界(库以源码分发); 唯一可见影响 = `sizeof(et_task_t)` 增大(8B/32 位) —— 静态数组/嵌入结构自动获得新大小; `--diff` 保持绿(类型名未变) |
