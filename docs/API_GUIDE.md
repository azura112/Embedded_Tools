...
**大小写不敏感变体（v2.0 决议：做折叠，不做通配）**：`et_smap_put_ci/get_ci/del_ci` 把键经
ASCII 折叠（`A`-`Z`→`a`-`z`）后走常规路径——**折叠形式入共用表/池**，与敏感条目共表共存
（`put("LED")` 与 `put_ci("LED")` 是两个条目）。通配/模式查找经评估"需求未证实"，
v2.0 终局关闭（[v3-candidates.md](v3-candidates.md)）。

**FAQ：为什么 et_map 维持键 0 保留、不做"全 u32 键域"？**（v1.9 决议）
空槽/墓碑标记是**实现事实**而非文档约定：键 0 与 0xFFFFFFFF 参与槽状态编码，改键域需要重设计状态标记（如独立状态字，槽体积 +4B 或引入位图双结构），收益不成比例。确有需要键 0 的应用走 **key+1 偏移**（应用侧一行数学），库不提供变体。u32 全键域且可区分删除的容器需求 = 字符串键需求由 et_smap 覆盖。**v2.0 终局关闭**：u32 全键域容器不再立项（[v3-candidates.md](v3-candidates.md) #6）。
...
- **读应答帧长不信任噪声字节（v2.6 CO-6）**：收到读类功能码时，其**字节数域必须严格等于 `2*exp_qty`**（单事务语义下主站已知期望寄存器数），不符即判为噪声 → **逐字节重同步**（不计 `crc_err`，因为从未成帧）。只有**帧首 = 本站地址且功能码/字节数域与在途事务严格同形**的序列 CRC 失败时，才计 `crc_err` 并按帧长整帧丢弃。此判据使"**主站自身请求回显**"（单线半双工最常见的噪声形态，形如 `11 03 00 00 00 04 …`）不再伪造帧长、不再污染统计，紧随其后的真应答可被正常解析 —— v2.5 行为下该形态只能靠超时重发收场。
...
| `retries` | 重发次数（不含首帧） |
| `crc_err` | **与在途事务严格同形**（帧首 = 本站地址且功能码/字节数域与期望一致）却 CRC 校验失败的候选帧数 —— v2.6 CO-6 起**不再计入从未成帧的噪声字节** |
| `addr_mismatch` | 地址不符被丢弃的候选帧数 |
| `late` | 非 BUSY 状态下被丢弃的**喂入突发数**（陈旧字节） |
| `discarded` | 其它丢弃：功能码不符 / **字节数域非法（噪声逐字节重同步的每一步）** / 回显不符 / 缓冲溢出 |

单测 **32 例**（`test/test_modbus_master.c`：0x03/0x04/0x06/0x10 正常流、分片应答、超时重发耗尽与重发后成功、异常不重试、广播写不等应答、迟到不污染、CRC 坏/地址不符丢弃、垃圾重同步、qty 边界、缓冲下限、多实例、统计累积、未初始化拒绝；**v2.6 CO-6 增 7 例噪声重同步**：自身请求回显形态、裸前缀 `[addr][读 fc]`、字节数域奇数/超上限、与在途事务严格同形的坏帧（`crc_err` 计数门槛）、qty=125 上限下的大帧重同步、写路径噪声前缀）；
...
| 类别 | 规格 | 行为 |
|---|---|---|
| 支持 | `%d %i %u %x %X %c %s %p %%` | 完整格式化（`%c` 的**域宽与左对齐**自 v2.6 CO-7 起生效；按 C 语义 `%c` 的精度不起作用） |
| 修饰 | 标志 `-` `0` `+` 空格 `#`；域宽（十进制，含 `*` 从实参取）；精度（`.N` / `.*`）；长度 `h` `hh` `l` `ll` `z` | 语义与 C `printf` 一致：`%04u` 补零、`%-5u` 左对齐、`%+d`/`% d` 符号、`%#x` 的 `0x` 前缀、`%.3d` 最少位数、`%.Ns`/`%.*s` 截断、`%lld`/`%zu`/`%hu` 长度 |
| 已知不支持（**按 C 语义消费实参** + 可见占位） | `%o %f %F %e %E %g %G %a %A`（按 `double` 消费）、`%n`（消费指针，**禁写内存**）、`%lc` / `%ls`、长度修饰 **`j` `t` `L`**（v2.6 CO-8 起：按 `intmax_t` / `ptrdiff_t` / `long double` 各消费一次） | 输出可见占位 —— 单字符转换 `%f` → `<?f>`、`%lc` → `<?lc>`；**含长度修饰者输出完整字形**（`%jd` → `<?jd>`、`%tu` → `<?tu>`、`%Lf` → `<?Lf>`） |
| 未知转换字符（**不消费实参**，例外） | 非上述集合，如 `%y`、`%Q` | 输出 `<?y>`、`<?Q>` 形态占位；**不消费** |
...
**v2.6 补的是什么（CO-7 / CO-8）**：① `%c` 的域宽/左对齐此前**被解析后丢弃**（`%5c` 输出 `A` 而非 `    A`）——现与 C 一致生效；
② 长度修饰 `j`/`t`/`L` 此前走"未知转换字符"分支**不消费实参**（错位的最后口子），现按 `intmax_t`/`ptrdiff_t`/`long double` 消费并给出完整字形占位。
至此**文档承诺的规格集合与实现的差集为空**（`et_log.h` 头注、本表、`test/test_log.c` 三者逐项一致）。
...
单测 `test/test_log.c` **30 例**（v2.5 起：每条规格用例**后随哨兵实参**断言取值正确以检出错位，受支持规格与 host `snprintf` **逐字节对拍**，不支持规格的占位与不错位、域宽上限、`%%`/尾部 `%` 边界；v2.6 CO-7/CO-8 起增 `%c` 域宽/左对齐/星号宽度的**逐字节对拍**、`j`/`t`/`L` 消费后哨兵取值正确、以及"输出中无字面 `%j`/`%t`/`%L` 残留"的负向断言）。
...
v2.0 追加可选 **Tab 命令名补全**（`ET_SHELL_TAB=1` 编译期开启，默认关闭=行为零变化）：
行首命令段（无空格、`AT+` 前缀）做**大小写敏感**前缀匹配（与 atcmd 名称规则一致）——
唯一匹配补全并回显；多候选换行列出全部命令名后重绘；无匹配/参数段/ESC 污染行则响铃或静默。
只补命令名：无路径补全/无历史模糊匹配（永久边界，见 [v3-candidates.md](v3-candidates.md) #7）。
测试矩阵双形态：`make test`（关闭态直透断言）+ `make test-tab`（开启态 7 例，CI 常设）。
...
| `ET_SHELL_TAB` | 0 | shell Tab 命令名补全开关（v2.0）：0=TAB 直透零变化；1=命令段大小写敏感前缀补全（唯一/多候选列表/无匹配响铃） |
