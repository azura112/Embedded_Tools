...
| 8 | 定点 Q15 双二阶(biquad)滤波器 | **维持排队, v2.5 复评**(v2.4 P0-2 判定, 2026-09-12): 立项条件 = 真实 IIR 型频响需求(陷波/高通截止/峰值 EQ)且 medfilt+lpf1 两级链不够用。**判定依据**: ① 库内两个 IIR 形态需求(ADC 去噪/微分前滤波)均已被 et_lpf1 覆盖, 无二阶频响诉求; ② 截至 v2.4 无任何真实用例记录(示例/板上走单/问题记录中均未出现陷波/高通场景); ③ 条件未触发则不消耗版本容量(Non-goals)。复评触发信号: 用户提交具体频响指标(截止频率/阻带衰减/采样率)。实现预案: 直接 IIA 转置型 + int64 累加 + 系数 Q15, 语义与 et_lpf1 同族, **不做浮点变体** |
